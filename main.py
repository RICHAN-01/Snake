...
music_dir = path.join(path.dirname('music_dir'), 'music')
img = pygame.image.load(path.join(music_dir, 'snake_background.png')).convert()
img = pygame.transform.scale(img, (dis_width, dis_height))
img_rect = img.get_rect()
game_over = pygame.image.load(path.join(music_dir, 'snake_picture.jpg')).convert()
game_over = pygame.transform.scale(game_over, (dis_width, dis_height))
...
am.set_volume(0.2)
hit = pygame.mixer.Sound(path.join(music_dir, 'удар.ogg'))
hit.set_volume(1)
hit_2 = pygame.mixer.Sound(path.join(music_dir, 'удар_об_стену.ogg'))
hit_2.set_volume(1)
...
def gameloop():
    ...
    while run:
        while game_close:
            ...
            create_mes(f'{length - 2}', WHITE, 320, 30, 'PressStart2P-Regular', 55)
